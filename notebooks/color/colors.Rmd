...
###### R::humanVerse #####
includeGithubFolder(github.monte.humanVerse, force.download = FALSE); 
```
...
###### R::humanVerse #####
includeGithubFolder(github.monte.misc, force.download = FALSE);
```
...
Notice, we may get some "rounding" error.  Or it could be a bug.
HSV and HSL are very similar.  They will create slightly different results, as two lines of code are different in the formulas.
```{r}
###### R::humanVerse #####
(rgb = unlist ( hex2rgb( wsu.crimson ) ) );
cat("\n", "===============", "\n\n");
...
color.displayColorOptions( c(wsu.crimson.neighbors, wsu.crimson),
                                                  showHEX = TRUE);
```
...
color.displayColorOptions( c(wsu.gray.neighbors, wsu.gray),
                                                  showHEX = TRUE);
```
