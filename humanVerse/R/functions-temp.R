...
units.createDictionary = function(memory="-UNITS_DICTIONARY-") 
	{
	d = memory.get(memory, "SYSTEM");
	return(minvisible(d, print=FALSE));
	if(is.null(d))
		{
		# FROM = a*x ... TO = x/a ... 
		
		dict = list();
		dict$search = list(); # keyed by variant ...
		
		
		type = "LENGTH";  	# 4 letters will be examined 
			variants = c("length", "len", "distance", "dis");
		dict$search = list.mapNtoOne(dict$search, variants, type);
		dict[[type]] = list();
			dict[[type]]$meta = list("type"=type, "variants"=variants);
			dict[[type]]$keys = list();
			dict[[type]]$search = list();
			
			key = "METER"; # 3 letters will be examined ...
			val = 1;
			var = c("m", "met")
			
			key = "AU";  
			val = 149597900000;
			var = c("ua", "au", "aus", "ast", "uni-ast", "ast-uni");
...
units.addKeyToDictionary = function(key="in", to="inches", type="distance", memory="-UNITS_DICTIONARY-") 
	{
