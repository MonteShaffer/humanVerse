...
# cbind(x,y)
# identical(x,y);  # one is integer 

messi = function(nsim=1000,			
							n = 1000, 	
							p.left = 0.6, 
							p.goalie.same = 0.5, 
							p.goal.same = 0.3,
	...
	{
	# mes = messi(nsim=1000, n=100, p.left=0.8, p.goal.same=0.05, p.goal.Nsame=0.95); mean(mes); hist(mes);  # .50100
	# if 0.05 + 0.95 == 1, mean = p.goalie.same 
	# answer is :: (p.goal.same + p.goal.Nsame)/2
	#  (p.goal.same + p.goal.Nsame) * p.goalie.same
	
	res = numeric(nsim);
	for(i in 1:nsim)
		{
		# https://stats.stackexchange.com/questions/339459/r-difference-between-rbinom-and-sample
		# 
			# pleft = rbinom(1, n, p.left);
		pleft = rbinom(n, 1, p.left); # ... actual data 
		pgsam = rbinom(n, 1, p.goalie.same);
		
		are.same = sum(pleft == pgsam);
		pgoals = p.goal.same  * sum(pleft == pgsam);
		pgoaln = p.goal.Nsame * sum(pleft != pgsam);
		
		goal = pgoals + pgoaln;
		res[i] = goal/n;
		}
	print( (p.goal.same + p.goal.Nsame)/2 );
	res;	
	}

# mes = messi(nsim=1000, n=100, p.left=0.8, p.goal.same=0.25, p.goal.Nsame=0.9, p.goalie.same=0.8); mean(mes); hist(mes); 
...
for(i in 1:n)
	{
	mes = messi(nsim=10000, n=1000, p.left=0.8, p.goal.same=0.25, p.goal.Nsame=0.9, p.goalie.same=pseq[i]);
	res[i] = mean(mes);
	}
	# (p.left) OR  (1-p.left) ... AND ...(p.goalie.same)  OR  (1-p.goalie.same)
	# -0.39 * p.goalie.same + 0.77
	# -0.39 * pseq + 0.77
	# p.left=0.8, p.goal.same=0.25, p.goal.Nsame=0.9
plot(pseq, res);
x = as.data.frame( cbind(pseq, res) );
x.lm = lm(res~pseq, data=x);
summary(x.lm);
